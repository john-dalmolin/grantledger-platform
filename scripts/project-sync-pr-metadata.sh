...
LIMIT="${LIMIT:-200}"
DRY_RUN="${DRY_RUN:-0}"
ONLY_OPEN="${ONLY_OPEN:-0}"      # 1 = processa só PRs OPEN
ONLY_WAVE="${ONLY_WAVE:-}"       # ex.: "Wave 5"
ONLY_PR_NUMBER="${1:-}"          # opcional: processa só 1 PR
...
apply_if_changed() {
  ...
  if [[ "$DRY_RUN" == "1" ]]; then
    echo "DRY_RUN: PR #$pr_number field=$field_name ${current_value:-<empty>} -> ${target_value:-<empty>}"
  else
...
while IFS= read -r row; do
  ...
  pr_json=$(gh pr view "$pr_number" --repo "$repo" --json title,state,isDraft,mergedAt,labels,milestone 2>/dev/null || true)
  if [[ -z "$pr_json" ]]; then
    echo "WARN: unable to read PR #$pr_number ($repo)"
    warnings=$((warnings + 1))
    skipped=$((skipped + 1))
    continue
  fi
  ...
  pr_milestone=$(echo "$pr_json" | jq -r '.milestone.title // ""')

  if [[ "$ONLY_OPEN" == "1" && ( -n "$merged_at" || "$state" == "CLOSED" || "$state" == "MERGED" ) ]]; then
    echo "SKIP: PR #$pr_number (closed, ONLY_OPEN=1)"
    skipped=$((skipped + 1))
    continue
  fi
  ...
  if [[ -z "$effective_milestone" ]]; then
    effective_milestone="$pr_milestone"
  fi
  ...
  if [[ -n "$ONLY_WAVE" && "$inferred_wave_for_filter" != "$ONLY_WAVE" ]]; then
    echo "SKIP: PR #$pr_number (wave=$inferred_wave_for_filter, ONLY_WAVE=$ONLY_WAVE)"
    skipped=$((skipped + 1))
    continue
  fi
  ...
  check_changed_field() {
    ...
    if [[ "$current_value" != "$target_value" ]]; then
      ...
      if [[ -z "$opt" || "$opt" == "null" ]]; then
        echo "WARN: option not found -> field=$field_name value=$target_value (PR #$pr_number)"
        missing=1
      fi
    fi
  }
  ...
  if [[ "$changed_candidates" -eq 0 ]]; then
    echo "SKIP: PR #$pr_number (no changes)"
    skipped=$((skipped + 1))
    continue
  fi
  ...
  if [[ "$DRY_RUN" == "1" ]]; then
    ...
    echo "UPDATED: PR #$pr_number changed_fields=$changed_fields"
  fi
